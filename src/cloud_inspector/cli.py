...
def cli(ctx: click.Context, log_level: str, project: str) -> None:
    ...
    ctx.obj["langsmith"] = langsmith_client
...
@click.pass_context
def list_prompts(ctx: click.Context, tag: Optional[str], service: Optional[str], format: str):
    ...
    if service:
        available_prompts = [p for p in available_prompts if p['service'].lower() == service.lower()]
...
def generate(ctx: click.Context, prompt_name: str, model: str, var: tuple[str, ...]):
    ...
    # Execute workflow
    flow = ctx.obj["workflow"]
    result = flow.execute(prompt_name, model, variables)
    
    if result.success:
        click.echo("\nCode Generation Successful!")
        click.echo("=" * 80)
        click.echo(result.code_result.code)
        click.echo("\nMetadata:")
        click.echo("-" * 80)
