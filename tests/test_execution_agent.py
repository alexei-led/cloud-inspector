...
def test_execution_agent_basic(code_executor):
    ...
    files = GeneratedFiles(main_py="import json\nprint(json.dumps({'test': 'success'}))", requirements_txt="", policy_json="")

    result = code_executor.execute_generated_code(generated_files=files, execution_id="test-123")
...
def test_execution_agent_invalid_json(code_executor):
    ...
    files = GeneratedFiles(main_py="print('not json')", requirements_txt="", policy_json="")

    result = code_executor.execute_generated_code(generated_files=files, execution_id="test-456")
...
""",
    ...
    result = code_executor.execute_generated_code(generated_files=files, execution_id="test-789")
